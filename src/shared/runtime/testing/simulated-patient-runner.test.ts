...
import { runSessions01To08Audit } from "@/shared/runtime/testing/simulated-patient-runner";

describe("Sessions 03-08 simulated patient audit (S01/S02 are prompt-driven)", () => {
  beforeEach(async () => {
    const db = getLocalDb();
  ...
  it("completes every reachable normal path with source-specific Program turns", async () => {
    ...
    expect(reports.map((report) => report.sessionId)).toEqual(["tbct-s03", "tbct-s04", "tbct-s05", "tbct-s06", "tbct-s07", "tbct-s08"]);
