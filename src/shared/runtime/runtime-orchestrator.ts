import type { ClinicalProviderResponse } from "@/shared/clinical-language/clinical-language-contract";
import { makeId } from "@/shared/id";
import type { ClinicalStageNode, PromptItem } from "@/shared/protocol/source-fidelity-types";
import { compileRuntimePrompt } from "@/shared/runtime/runtime-prompt-compiler";
import { reduceRuntimeState, type RuntimeStateReduction } from "@/shared/runtime/runtime-state-reducer";
import type { RuntimeActiveStep } from "@/shared/runtime/runtime-step-resolver";
...
export async function orchestrateRuntimeAssistantTurn(input: RuntimeOrchestratorInput): Promise<RuntimeOrchestratorResult> {
  ...
  if (isDialogueAgentEnabled(input.session.sessionDefinitionId)) {
    ...
    // runtimeContext). summaryCheck is never set on a fallback, and a
    // repeated-fallback override only ever replaces fallback text, so the
    // text that opens the check is always the summary itself.
