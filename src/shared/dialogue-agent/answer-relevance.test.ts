...
// Off-topic answers and field corrections (.claude/TASK_SCOPE.json
// note2026_09_14_off_topic_answers, note2026_09_14_field_corrections): the
// check that runs before a free-text answer is stored, and fails open.
...
describe("judgeAnswerRelevance", () => {
  ...
  it("returns Claude's verdict, asking with the question actually shown and the participant's message", async () => {
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(toolResponse({ verdict: "off_topic", reason: "talks about lunch, not a feeling" }));
    expect(await judgeAnswerRelevance(request, context)).toEqual({ isAnswer: false, checked: true, verdict: "off_topic", reason: "talks about lunch, not a feeling" });
  ...
  it("treats 'nothing more' as an answer and a correction request as not one", async () => {
    process.env.ANTHROPIC_API_KEY = "sk-ant-test";
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(toolResponse({ verdict: "stop", reason: "typo of 없어요" }));
    expect(await judgeAnswerRelevance({ ...request, answer: "읎오" }, context)).toMatchObject({ isAnswer: true, checked: true, verdict: "stop" });
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(toolResponse({ verdict: "correction_request", reason: "wants to remove an earlier item" }));
    expect(await judgeAnswerRelevance({ ...request, answer: "아까 그건 잘못 쓴 거예요" }, context)).toMatchObject({ isAnswer: false, checked: true, verdict: "correction_request" });
  });
