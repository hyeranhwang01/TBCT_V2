...
export function PatientProfilePage() {
  ...
  if (participantQuery.isLoading) return <PatientShell title={t("patientProfile.title")}><PageSkeleton /></PatientShell>;
  const participant = participantQuery.data;
  if (!participant) return <PatientShell title={t("patientProfile.title")}><Card><EmptyState title={t("patientProfile.notFound")} /></Card></PatientShell>;
  return (
    <PatientShell title={t("patientProfile.title")}>
      <div className="mx-auto grid w-full max-w-4xl gap-4">
        <Card className="p-5 sm:p-6">
          <div className="text-lg font-bold text-text-primary">{t("patientProfile.edit.title")}</div>
